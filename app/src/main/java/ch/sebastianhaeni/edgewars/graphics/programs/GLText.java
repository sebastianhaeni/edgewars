...
public class GLText {
    ...
    private BatchTextProgram mProgram;                           // OpenGL Program object

    //--Constructor--//
    // D: save program + asset manager, create arrays, and initialize the members
    public GLText(BatchTextProgram program, Context context) {
        if (program == null) {
            program = new BatchTextProgram(context);
        }
        this.assets = context.getAssets();                           // Save the Asset Manager Instance
    ...
    void initDraw(float red, float green, float blue, float alpha) {
        ESShader.checkGlError("ss");

        GLES20.glUseProgram(mProgram.getProgramHandle()); // specify the program to use
        ESShader.checkGlError("glUseProgram");

        // set color TODO: only alpha component works, text is always black #BUG
        float[] color = {red, green, blue, alpha};
        GLES20.glUniform4fv(mProgram.getColorHandle(), 1, color, 0);
    ...
    public void end() {
        ...
        GLES20.glDisableVertexAttribArray(mProgram.getColorHandle());
